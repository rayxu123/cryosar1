...
Ray Xu
July 2023
CryoSAR1/SControl/SControl_DAC_GUI.py
...
class SControl_DAC_GUI(QMainWindow):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.setWindowTitle("AD5313 DAC Control")
        ...
        # Initiate hardware
        self.spi = ftdispi.ftdispi(self.args.addr, self.args.noConnect, mode=1)
        # Set LDAC high
        self.spi.setgpio(1)
        # Send programming twice to flush out possible garbage in the first programming
        self.ftdiProg()
        self.ftdiProg()
        '''
    ...
    def populateGUI(self):
        ...
        self.disableBtn.clicked.connect(self.ftdiDisable)
        # Read and verify are not implemented for AD5313
        ...
        ftdiLayout.addWidget(self.disableBtn, 0, 3)
    ...
    # Disables FTDI device
    # May take up to 10 seconds (tested few times on win 10, python 3.11)
    def ftdiDisable(self):
        # Close FTDI first
        self.spi.close()
        # Permanently disable other actions
        self.verifyBtn.setEnabled(False)
        self.readBtn.setEnabled(False)
        self.progBtn.setEnabled(False)
        self.loadBtn.setEnabled(False)
        self.saveBtn.setEnabled(False)
        for obj in self.menuList:
            obj.setEnabled(False)
        

    # Reads from FTDI SPI
    def ftdiRead(self):
        # Not implemented for AD5313   
        pass     
        '''
    ...
    def ftdiProg(self):
        ...
        self.spi.setgpio(1)
        time.sleep(0.01)
        # Slice bitstring into halves since A and B cannot be programmed simultaneously for a given chip
        bitstring = self.cfg.toBits()
        firstpart, secondpart = bitstring[:len(bitstring)//2], bitstring[len(bitstring)//2:]
        #print("A bits:  "+firstpart)
        #print("B bits:  "+secondpart)
        # Program A half then B half
        # Why B half needs to be programmed twice?
        self.spi.progAD5313half(firstpart)
        self.spi.progAD5313half(firstpart)
        time.sleep(0.01)
        self.spi.progAD5313half(secondpart)
        self.spi.progAD5313half(secondpart)
        # Clear highlights from changing menu items
        for ptr in self.ptrList:
            self.highlightMenuElement(ptr, False)
        # Update previous bits variable
